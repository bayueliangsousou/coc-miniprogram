...
- **进入编辑页时，职业配置里明确指定的技能（固定锁定 + 选多候选）必须全部标★**，让用户一眼识别"哪些是我能加职业点的技能"。绝不是"加到50才挣星"。

**统一交互模型（重构目标）**：
| 机制 | 进入时★ | 加点后 |
|---|---|---|
| 固定锁定（baseline） | ★ | 恒★ |
| 选多候选（chooseFrom，含社交四选一/下列选四/艺术任一/科学/互斥） | **全部★** | 超过50的前 M 个保持★（占职业名额），其余摘星 |
| 全选多（chooseAny，点X门技能/自选X技能） | 不标（无预设候选） | 用户加到50+才★ |

- 互斥（二选一）= `chooseFrom{count:1}` 特例：进入双★，给一方加到50另一方摘星。
- 选多名额约束的是"谁真正占职业点池"，而非"是否有资格标★"；★的初始全亮是"资格提示"。

**当前代码错误（待重构修复）**：
- `categoryLimits`（按分类名，如「一项社交技能」→社交=1）与 `listLimits`（按名单，如罪犯「下列选四」）走"加法模型"：候选初始不★、到50才挣星、且按分类名覆盖全分类（范围过宽易串味）。这与互斥"减法模型"（初始★、超额摘星）相反，是社交技能被吐槽"逻辑不对"的根因。
- 重构方案 B：收敛为 `chooseFrom`（候选默认★、超50前M留星）+ `chooseAny`（全局选M、加到50才★）+ baseline 固定锁定，互斥为 `chooseFrom{count:1}` 特例。职业数据改为纯声明式（`skills` + `chooseFrom[]` + `chooseAny`），机制层不随职业膨胀。

**范围边界（用户 2026-07-12 最终确认）**：
- **chooseFrom（选多候选：互斥 / 社交四选一 / 下列选四 / 艺术任一 / 科学）统一走减法模型**：进入时全部★，加点后超50降序前 M 个留星、其余摘星。社交四选一与互斥体验一致。
- **chooseAny（全技能选N / 点X门技能 / 自选X技能）保持旧逻辑不变**：进入时不标★、用户加到50+才★（旧加法模型）。不重构。
- 重构范围收窄为：仅把 `categoryLimits` / `listLimits` 从加法改减法（并入 chooseFrom），`optionalCount` 机制与显示完全不动。
