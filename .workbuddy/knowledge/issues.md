...
### 局部选n / 全局选N：第 N+1 个技能不能超 50，且系统绝不替用户改已填值（两层修复）
- 现象：社交/艺术/科学「选N」（`categoryLimits`）/ 自由选N（`chooseAny`）职业里，玩家能把第 3 个同类技能加到 >50，违反「局部选n = 该分类最多 N 个职业技能，其余是兴趣技能封顶 50」。
- 根因（分两层）：①显示层——原 `updateSkillDisplayState` 用【按值降序】取前 N，且 chooseAny 候选收集未排除「处于有限分类内、只是超名额」的技能，导致第 3 个社交技能能被自由选 N 提拔为★、且数值高的后来者会按值挤掉先填的；②落盘层——即便显示层已把第 N+1 个技能判为非★，`onSkillInput` 实时不截断、`onSave`/`onHide` 保存路径直接读 `sk.current` 落盘、不跑校验，于是填完没失焦就保存/切页时 60 原样存进存档，重载后第 3 个技能稳稳停在 60（只是没★）。
- 修复（两层）：①显示层——`categoryOccupied`/`globalOccupied` 均改为「按分类→技能**出现顺序**取前 N 个 >50 的非锁定技能」，数值高的后来者不挤先填的；chooseAny 候选收集排除「处于有限分类、超名额」的技能；`chooseFrom`（名单型选多）保留按值降序取最优 N（与选N 是两套模型，用户已确认）。②落盘层——新增 `clampOverflowSkills(skillCategories)`：保存/落草稿前调 `updateSkillDisplayState` 取最新 ★ 标记，对「非★ 且 current>50 且 非母语」的技能归 50（重算 hard/extreme）；`onSave`（附 toast「已自动将 X 个超出职业上限的技能归为50」）、`onHide`（先 setData 再存）两处调用；`_doSave` 改走统一 `collectSkills`。
- 铁律：系统**绝不**替用户降低「已占名额的 N 个职业技能」的值（不挤下）；第 N+1 个技能拿不到职业技能点（★），但**只有用户自己把前面的降到 50 以下，后面的才腾出名额**；落盘时第 N+1 个超编值由 `clampOverflowSkills` 归 50（这是保存兜底，不是实时改用户正在编辑的值）。母语按设计豁免（固定高基础值、非职业选技）。涉及文件：`pages/skills/skills.js`。
...
## 物理骰子调研（ISSUE-DICE-001 ~ 005，2026-09-24）
...
| 骰子 | 卡方 | 判定 | 根因 |
|---|---|---|---|
| **D4** | **1639** | ❌ 严重（1 占 75.6%） | 物理凸包质心错位 → 不倒翁效应 |
| **D6** | 15.96 | ❌ 轻微 | 初态用欧拉角，姿态分布不均匀 |
| **D3** | 13.95 | ❌ 轻微 | 同上（与 D6 共用 Box 物理） |
| D8 | 8.79 | ✅ | — |
| D10 | 6.55 | ✅ 点数正常 / 🟡 凸包上半 10 面法线朝内 | cannon 启动打印 10 条警告 |
| D12 | 8.60 | ✅ 点数正常 / 🟡 视觉只有物理的 78% | 尺寸不符 → 骰子浮空 |
| D20 | 15.87 | ✅ 点数正常 / 🟡 判定表顶点不是正二十面体 | 第5近邻距离 0.842~0.963 不等长 |
| D100 | — | 🟠 规则 bug | `tens+unit` 出 00+0 返回 0，应为 100（1% 概率） |

### ISSUE-DICE-001【P0】D4 点数 1 占 76.7%

- **根因**：`createConvexBody` D4 顶点原点在**底面中心**，而正四面体重心在 `h/4`；cannon-es **直接用 body 局部原点当质心** → 等效「底面灌铅」。
- **不倒翁判据**：底面贴地时质心离地 0.0030，其余姿态 0.3132（差 104 倍）；该姿态恰被读成点数 1。
- **物理/判定分流证明判定层无罪**：物理贴地面↔判定值一一对应（面3→1、面1→2、面2→3、面0→4）。
- **修复（已验证）**：凸包顶点平移到重心 `cg = h/4` → 1 从 76.7% 降到 26.1%，卡方 3562→**3.4 ✅**；`createD4Mesh` 视觉顶点**必须同步平移**（否则渲染比物理低 0.245）。判定表可不同步（整体平移不改变排序，20000 次验证结果相同）。
- **对照**：与三角机构修复前完全同源（三角机构已修并真机验收通过）。

### ISSUE-DICE-002【P1】D6 / D3 轻微偏斜

- **根因**：`mesh.rotation.set(随机,随机,随机)` 的三随机欧拉角**不是均匀分布**（极点聚集）。
- **修复（已验证）**：改用均匀随机四元数（Shoemake）→ 卡方 16.0/12.7 降到 **1.8/1.8 ✅**。
- **附带**：`snapToGround()` 对 D6/D3 **完全失效**——cannon-es `Box` 没有 `.vertices`，源码 `if(!shape.vertices) return` 直接返回。

### ISSUE-DICE-003【P2】D10 凸包上半法线朝内

- 上半（含顶点 0）10 个三角面绕序相反，cannon 启动打印 10 条 `points into the shape` 警告。
- 对点数分布无影响（卡方 6.55 ✅），但 SAT 碰撞依赖法线，边缘情况可能穿透/卡墙角。

### ISSUE-DICE-004【P2】D20 判定表顶点畸形

- `VERTEX_DATA.D20` 12 个顶点与 mesh/物理**不是同一组点**（最近邻 0.4594）；第5近邻距离不等长、距离种类 7 种（正二十面体应为 3）。
- 但读数检查 **20/20 全部一致**（面索引与数字按顺序对齐）→ 日常读数正确，非标准姿态下有 31% 锚点差 / 10.9° 方向偏差的误读风险。

### ISSUE-DICE-005【P3】D100 取值 0

- `var dv = tens + unit` → 值域 0–99；CoC 规则 00+0 = 100。概率 1%，会导致检定误判。
...
- 初版修复验证脚本把 D4 平移顶点写成 `[r,0,0]`（漏减 `cg`），误得「修复后仍偏 38%」；修正后确认卡方 3.4 ✅。以此为准。
- 曾怀疑 D20 判定表导致读数错误 → 实测 20/20 一致，降级为隐患。
